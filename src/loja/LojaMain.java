    ...
    public void conectarNaFabrica() {
        new Thread(() -> {
            try (Socket socket = new Socket("localhost", 12345)) {
                ObjectOutputStream outIdentificacao = new ObjectOutputStream(socket.getOutputStream());
                outIdentificacao.writeInt(this.lojaId);
                outIdentificacao.flush();
    ...
    private void processarVenda(Socket clienteSocket) {
        try (ObjectOutputStream out = new ObjectOutputStream(clienteSocket.getOutputStream())) {
            ...
            if (v != null) {
                out.writeObject(v);
                out.flush();
                System.out.printf("LOG VENDA CLIENTE - Loja: %d | Veículo: %s | Status: Entregue%n",
                        lojaId, v.toString());
            }
        } catch (Exception e) { }
    }
